...
For the HTML and PDF I also included an NPS banner in the report to show how to make that work. Note that the arrowhead image is missing from the repo (I don't think we can share these publicly). You'll need to specify your own version of that for these to work. The NPS_banner.html that feeds into the HTML report is also in this folder, along with the example_styles.css. Note that that LaTeX code to make the banner in the PDF version is NOT pretty. Revisiting this header for the training reminded me why I decided never again to work in LaTeX a few years ago.

I couldn't include an NPS banner for the output to Word. It's very possible `officedown` has a function to make that work, but I didn't have a chance to look into it. I also couldn't get cross-referencing to work with the figures. Again, `officedown` is likely the answer. I just ran out of time troubleshooting it.
...
<li><a href = "https://github.com/NCRN/ForestVegIAR">NCRN Forest Vegetation Investigator's Annual Report</a></li>
