...
If you run this on smaller datasets or faster tasks, you may see little gain or you may even discover that it is slower. This is because there is 'overhead' involved in initiating and sending tasks to threads so this only returns a benefit when the individual task takes longer than the sum total of overhead required to run a parallel session.
