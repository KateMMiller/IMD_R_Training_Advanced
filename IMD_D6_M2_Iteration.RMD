...
Fix_Data <- function(data) {
  data %>%
    ...
    mutate(Temp_C = 5 * (Temp_F - 32) / 9, Date_Time = mdy_hms(Date_Time), Date=date(Date_Time),Hour = hour(Date_Time))
}
...
Gamma_Reg <- summary_data %>%
  map_if(~ all(c("Intensity", "Temp_C") %in% colnames(.x)),
    ~ Intensity_Regression(.x, family = Gamma),
    .else = ~NA
  )

```

This throws an error because the Gamma distribution assumes there are no 0s in your data. We could go then check if 0 is a realistic value for intensity and maybe do some QA/QC for filtering, but for now let's just move on. 
