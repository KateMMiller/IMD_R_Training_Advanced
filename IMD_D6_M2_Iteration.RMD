...
## Saving Time and Effort with Custom Functions ##
...
```for()``` loops run the same code a certain number of times. 
...
b)  Loops, especially nested loops can be hard to read and debug if you come back to the code after a long time
c) R has other options that are faster and easier to read
...
First, I will read in all the data.  ```set_names()``` is used to name the ```fPaths``` vector, and those names will be carried forward to the data list. The ```map``` funtcion will take each element of ```fPaths``` and feed them one by one into ```read_csv```. The ```.x``` tells map where to put the data it gets from ```fPaths```. Note that you need to have a ```~``` in front of the file name. The advantage of doing iteratin this way is that it is more conscise than a ```for``` loop, but somewhat easier to read than the ```lapply``` function.
...
```
I used ```map_dbl``` instead of ```map``` to look at the number of rows. That funciton is just like ```map``` except it will return a numeric vector rather than a list. In this case it is handy as I already know that ```nrow()``` will return a number, at a vector is a more compact way of showing that. Note that all the lists and the elements of the row number vector are named with the site code. 
...
summary_data<-intensity_data %>% map(~.x %>% group_by(Date) %>% summarise(across(where(is.numeric), mean)))
